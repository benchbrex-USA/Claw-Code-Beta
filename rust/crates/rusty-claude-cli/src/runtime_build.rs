...
pub(super) struct RuntimePluginState {
    pub(super) feature_config: runtime::RuntimeFeatureConfig,
    pub(super) tool_registry: GlobalToolRegistry,
    pub(super) plugin_registry: PluginRegistry,
    pub(super) mcp_state: Option<Arc<Mutex<RuntimeMcpState>>>,
}

pub(super) struct RuntimeMcpState {
...
impl BuiltRuntime {
    ...
    pub(super) fn with_hook_abort_signal(
        mut self,
    ...
    pub(super) fn shutdown_plugins(&mut self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub(super) fn shutdown_mcp(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        if self.mcp_active {
...
impl RuntimePluginState {
    pub(super) fn into_tool_registry(self) -> Result<GlobalToolRegistry, Box<dyn std::error::Error>>
    {
...
impl RuntimeMcpState {
    ...
    pub(super) fn pending_servers(&self) -> Option<Vec<String>> {
        (!self.pending_servers.is_empty()).then(|| self.pending_servers.clone())
    }

    pub(super) fn degraded_report(&self) -> Option<runtime::McpDegradedReport> {
    ...
    pub(super) fn list_resources_for_server(
        &mut self,
        server_name: &str,
    ) -> Result<String, ToolError> {
        let result = self
    ...
    pub(super) fn list_resources_for_all_servers(&mut self) -> Result<String, ToolError> {
