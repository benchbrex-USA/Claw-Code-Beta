...
pub struct McpToolRegistry {
    ...
    manager: Arc<OnceLock<Arc<AsyncMutex<McpServerManager>>>>,
}
...
impl McpToolRegistry {
    ...
    pub fn set_manager(
        &self,
        manager: Arc<AsyncMutex<McpServerManager>>,
    ) -> Result<(), Arc<AsyncMutex<McpServerManager>>> {
...
mod tests {
    ...
    fn given_connected_server_with_manager_when_calling_tool_then_it_returns_live_result() {
        ...
        let manager = Arc::new(AsyncMutex::new(McpServerManager::from_servers(&servers)));
    ...
    fn call_tool_payload_structure() {
        ...
        registry
            .set_manager(Arc::new(AsyncMutex::new(McpServerManager::from_servers(
                &servers,
            ))))
            .expect("manager should only be set once");
