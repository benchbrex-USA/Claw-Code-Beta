...
pub struct Worker {
    ...
    pub status: WorkerStatus,
    #[serde(flatten)]
    pub trust: WorkerTrustState,
    pub prompt_delivery_attempts: u32,
    #[serde(flatten)]
    pub prompt: WorkerPromptState,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct WorkerTrustState {
    #[serde(rename = "trust_auto_resolve")]
    pub auto_resolve: bool,
    #[serde(rename = "trust_gate_cleared")]
    pub gate_cleared: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct WorkerPromptState {
    #[serde(rename = "auto_recover_prompt_misdelivery")]
    pub auto_recover_misdelivery: bool,
    #[serde(rename = "prompt_in_flight")]
    pub in_flight: bool,
}
...
impl WorkerRegistry {
    ...
    pub fn create(
        ...
        let mut worker = Worker {
            ...
            trust: WorkerTrustState {
                auto_resolve: trust_auto_resolve,
                gate_cleared: false,
            },
            prompt_delivery_attempts: 0,
            prompt: WorkerPromptState {
                auto_recover_misdelivery: auto_recover_prompt_misdelivery,
                in_flight: false,
            },
            last_prompt: None,
            replay_prompt: None,
    ...
    pub fn observe(&self, worker_id: &str, screen_text: &str) -> Result<Worker, String> {
        ...
        if handle_trust_prompt(worker, &lowered) {
            return Ok(worker.clone());
        }

        if handle_prompt_misdelivery(worker, screen_text, &lowered) {
            return Ok(worker.clone());
        }

        apply_running_cue(worker, &lowered);
        apply_ready_cue(worker, screen_text, &lowered);
    ...
    pub fn resolve_trust(&self, worker_id: &str) -> Result<Worker, String> {
        ...
        worker.trust.gate_cleared = true;
    ...
    pub fn restart(&self, worker_id: &str) -> Result<Worker, String> {
        ...
        let worker = inner
            ...
            .ok_or_else(|| format!("worker not found: {worker_id}"))?;
        worker.status = WorkerStatus::Spawning;
        worker.trust.gate_cleared = false;
        worker.last_prompt = None;
        worker.replay_prompt = None;
        worker.last_error = None;
        worker.prompt_delivery_attempts = 0;
        worker.prompt.in_flight = false;
        push_event(
            worker,
...
fn prompt_misdelivery_is_relevant(worker: &Worker) -> bool {
    worker.prompt.in_flight && worker.last_prompt.is_some()
}

fn handle_trust_prompt(worker: &mut Worker, lowered: &str) -> bool {
    if worker.trust.gate_cleared || !detect_trust_prompt(lowered) {
        return false;
    }
...
fn handle_prompt_misdelivery(worker: &mut Worker, screen_text: &str, lowered: &str) -> bool {
    let Some(observation) = prompt_misdelivery_is_relevant(worker)
        .then(|| {
    ...
    let message = match observation.target {
        ...
        WorkerPromptTarget::Unknown => {
            format!("worker prompt delivery failed before reaching coding agent: {prompt_preview}")
        }
    };
    ...
    if worker.prompt.auto_recover_misdelivery {
...
fn apply_running_cue(worker: &mut Worker, lowered: &str) {
    if detect_running_cue(lowered) && worker.prompt.in_flight {
        worker.prompt.in_flight = false;
        worker.status = WorkerStatus::Running;
        worker.last_error = None;
    }
...
fn apply_ready_cue(worker: &mut Worker, screen_text: &str, lowered: &str) {
...
mod tests {
    ...
    fn allowlisted_trust_prompt_auto_resolves_then_reaches_ready_state() {
        ...
        assert!(after_trust.trust.gate_cleared);
    ...
    fn trust_prompt_blocks_non_allowlisted_worker_until_resolved() {
        ...
        assert!(resolved.trust.gate_cleared);
    ...
    fn prompt_misdelivery_is_detected_and_replay_can_be_rearmed() {
        ...
        assert!(running.prompt.in_flight);
    ...
    fn restart_and_terminate_reset_or_finish_worker() {
        ...
        assert!(!restarted.prompt.in_flight);
